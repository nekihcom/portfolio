...
const NoteCard = (props: Props) => {
  ...
  const displayBlogPostDt = new Date(note.pubDate).toLocaleDateString('sv-SE');
  // デフォルト画像のURL
      ...
      >
        <div className="flex flex-col h-full">
          <div className="relative w-full h-48 overflow-hidden">
            <Image
              src={note.thumbnail || defaultImageUrl}
              alt={note.title}
              fill
          ...
          <div className="p-4 flex flex-col flex-grow">
            ...
            <h3 className="mt-2 text-sm font-bold line-clamp-2 hover:text-gray-600 transition-colors duration-300">{note.title}</h3>
          </div>
        </div>
      </Link>
    </article>
